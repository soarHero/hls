  ...
  host.processMetadata = function(type, data, timestamp) {
    if (type == 'ID3') {
      ...
      for (var i = 0; i < data.length - 1; i++) {
        id3str = id3str + String.fromCharCode(data[i]);
      }
      self.log_('id3 data received:' + id3str);
    } else {
      self.log_('processMetadata(): non-id3 data');
    }
  };
...
sampleplayer.CastPlayer.prototype.loadVideo_ = function(info) {
  ...
  if (!protocolFunc) {
    ...
    var metadataCallback = function(type, data, timestamp) {
      if (type == 'ID3') {
        var id3str = '';
        for (var i = 0; i < data.length - 1; i++) {
          id3str = id3str + String.fromCharCode(data[i]);
    ...
    if (!this.preloadPlayer_ || (this.preloadPlayer_.getHost &&
      ...
      host.processMetadata = metadataCallback;
      ...
      this.player_.getHost().processMetadata = metadataCallback;
      this.player_.load();
      wasPreloaded = true;
    }
  }
